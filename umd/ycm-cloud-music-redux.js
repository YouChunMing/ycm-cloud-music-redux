(function (global, factory) {
  ...
  (global = global || self, factory(global.YCMCloudMusicRedux = {}));
  ...
  */
  ...
  function get$2(key, x) {
    return isMap$1(x) ? x.get(key) : x[key];
  }
  ...
  var flattenWhenNode = (function (predicate) {
    return function flatten(map, _temp, partialFlatMap, partialFlatActionType) {
      ...
      ownKeys$1(map).forEach(function (type) {
        var nextNamespace = connectPrefix(connectNamespace(type));
        var mapValue = get$2(type, map);
  ...
  function handleAction(type, reducer, defaultState) {
    ...
    var types = toString$1(type).split(ACTION_TYPE_DELIMITER);
    ...
    return function (state, action) {
      ...
      if (!actionType || types.indexOf(toString$1(actionType)) === -1) {
        return state;
      }
  ...
  var flattenReducerMap = flattenWhenNode(function (node) {
    return (isPlainObject(node) || isMap$1(node)) && !hasGeneratorInterface(node);
  });
  ...
  function handleActions(handlers, defaultState, options) {
    ...
    invariant_1(isPlainObject(handlers) || isMap$1(handlers), 'Expected handlers to be a plain object.');
    var flattenedReducerMap = flattenReducerMap(handlers, options);
    var reducers = ownKeys$1(flattenedReducerMap).map(function (type) {
      return handleAction(type, get$2(type, flattenedReducerMap), defaultState);
    });
  ...
  var reducer = dist_1((_combineReducers = {}, _defineProperty(_combineReducers, STATE_BYID_REDUCER_KEY, byIdReducer), _defineProperty(_combineReducers, STATE_ALLIDS_REDUCER_KEY, allIdsReducer), _combineReducers));
  ...
  var reducer$1 = dist_1(_defineProperty({}, BANNERS_STATE_KEY, bannerReducer));
